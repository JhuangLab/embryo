...
To calculate the correlation between gene expression and metabolite abundance, we align the coordinates of the spatial transcriptome and the spatial transcriptome. The specific calculation methods have been described in the Method section. After centralizing the coordinates of the spatial transcriptome spots, we performed scaling, rotation, and translation transformations to make the spatial transcriptome spots as close as possible to the spatial transcriptome pixels. Since spatial transcriptomics and spatial metabolomics are from different omics, we only use the tissue shape for alignment. Although the alignment process was automated, manual validation was still required.
