...
## Figure 5 | Metabolic gradient and compartmantalizaiton patterns at the organ substructure level

This section loads required R packages and sets up the working directory.
```{r, eval = FALSE} 
...
## Figure 5b: Monocle2 Trajectory for Gene and m/z Data

This section generates spatial and trajectory plots for pseudotime based on gene and m/z data using Monocle2.
```{r, eval = FALSE} 
...
```
## Figure 5g: Signaling and Metabolic Pathway Co-occurrence

This section visualizes the co-occurrence of signaling and metabolic pathways in spatial plots for different samples.
```{r, eval = FALSE} 
# Define comparison dataframe for pathways
...
# Loop over samples to generate plots
for(samp in c("E115", "E135")) {
  obj1 <- obj_lst[[samp]]
  
  # Adjust coordinates based on sample
  if(samp == "E115") {
  ...
  # Generate plots for each pathway comparison
  plst3[[samp]] <- lapply(1:nrow(comp_df), \(idx) {
    ...
    # Base plot with signaling feature
    p <- ggplot2::ggplot() + 
      ggplot2::geom_point(
    ...
    # Add density contours and title
    p2 = p1 + 
      ggnewscale::new_scale_fill() +
      geom_density_2d(
        aes(x = x, y = y), 
...
# Combine plots into a patchwork layout
